...
/** RISCV standard SSDT Cpu Topology Table Generator.

Requirements:
  The following Configuration Manager Object(s) are required by
  this Generator:
  - ERiscVObjRintcInfo
  - ERiscVObjProcHierarchyInfo (OPTIONAL) along with
  - ERiscVObjCmRef (OPTIONAL)
  - ERiscVObjLpiInfo (OPTIONAL)
*/
...
/** Initialize the TokenTable.
  ...
  One entry should be allocated for each CM_RISCV_PROC_HIERARCHY_INFO
  structure of the platform. The TokenTable allows to have a mapping:
  Index <-> CM_OBJECT_TOKEN (to CM_RISCV_LPI_INFO structures).

  There will always be less sets of Lpi states (CM_RISCV_OBJ_REF)
  than the number of cpus/clusters (CM_RISCV_PROC_HIERARCHY_INFO).
...
/** Create and add an _CPC Node to Cpu Node.
  ...
  @param [in]  CfgMgrProtocol         Pointer to the Configuration Manager
                                      Protocol Interface.
  @param [in]  RintcInfo               Pointer to the CM_RISCV_RINTC_INFO object
                                      describing the Cpu.
...
CreateAmlCpcNode (
  ...
  IN  CM_RISCV_RINTC_INFO                                 *RintcInfo,
  IN  AML_OBJECT_NODE_HANDLE                              *Node
  )
{
  EFI_STATUS       Status;
  CM_RISCV_CPC_INFO  *CpcInfo;

  Status = GetERiscVObjCpcInfo (
             CfgMgrProtocol,
             RintcInfo->CpcToken,
             &CpcInfo,
             NULL
...
/** Create and add an _LPI method to Cpu/Cluster Node.
  ...
  @param [in]  Generator              The SSDT Cpu Topology generator.
  @param [in]  ProcHierarchyNodeInfo  CM_RISCV_PROC_HIERARCHY_INFO describing
                                      the Cpu.
...
CreateAmlLpiMethod (
  IN  ACPI_CPU_TOPOLOGY_GENERATOR  *Generator,
  IN  CM_RISCV_PROC_HIERARCHY_INFO   *ProcHierarchyNodeInfo,
  IN  AML_OBJECT_NODE_HANDLE       *Node
  )
{
  EFI_STATUS  Status;
  ...
  )
{
  ...
  CM_RISCV_OBJ_REF          *LpiRefInfo;
  UINT32                  LpiRefInfoCount;
  UINT32                  LpiRefIndex;
  CM_RISCV_LPI_INFO         *LpiInfo;
  ...
  for (Index = 0; Index < LastIndex; Index++) {
    ...
    for (LpiRefIndex = 0; LpiRefIndex < LpiRefInfoCount; LpiRefIndex++) {
      // For each CM_RISCV_LPI_INFO referenced by the token, add an Lpi state.
      Status = GetERiscVObjLpiInfo (
                 CfgMgrProtocol,
...
/** Create a Cpu in the AML namespace.
  ...
  @param [in]  RintcInfo    CM_RISCV_RINTC_INFO object used to create the node.
...
/** Create a Cpu in the AML namespace from a CM_RISCV_PROC_HIERARCHY_INFO
    CM object.
  ...
  )
{
  ...
  ASSERT (ParentNode != NULL);
  ASSERT (ProcHierarchyNodeInfo != NULL);
  ASSERT (ProcHierarchyNodeInfo->RintcToken != CM_NULL_TOKEN);

  Status = GetERiscVObjRintcInfo (
             CfgMgrProtocol,
             ProcHierarchyNodeInfo->RintcToken,
             &RintcInfo,
             NULL
             );
  if (EFI_ERROR (Status)) {
  ...
  Status = CreateAmlCpu (Generator, ParentNode, RintcInfo, CpuName, &CpuNode);
  if (EFI_ERROR (Status)) {
    ASSERT (0);
    return Status;
  }
  ...
  // CM_RISCV_PROC_HIERARCHY_INFO, create an _LPI method returning them.
  ...
  if (RintcInfo->CpcToken != CM_NULL_TOKEN) {
    Status = CreateAmlCpcNode (Generator, CfgMgrProtocol, RintcInfo, CpuNode);
    if (EFI_ERROR (Status)) {
      ASSERT_EFI_ERROR (Status);
      return Status;
    }
...
/** Create a Processor Container in the AML namespace.
  ...
  Any CM_RISCV_PROC_HIERARCHY_INFO object with the following flags is
  ...
  @param [in]  ParentNode             Parent node to attach the processor
                                      container node to.
  @param [in]  ProcHierarchyNodeInfo  CM_RISCV_PROC_HIERARCHY_INFO object used
                                      to create the node.
...
/** Create an AML representation of the Cpu topology.
  ...
  @param [in] CfgMgrProtocol          Pointer to the Configuration Manager
                                      Protocol Interface.
  @param [in] NodeToken               Token of the CM_RISCV_PROC_HIERARCHY_INFO
                                      currently handled.
  ...
  )
{
  ...
  for (Index = 0; Index < Generator->ProcNodeCount; Index++) {
    // Find the children of the CM_RISCV_PROC_HIERARCHY_INFO
    // currently being handled (i.e. ParentToken == NodeToken).
    if (Generator->ProcNodeList[Index].ParentToken == NodeToken) {
      // Only Cpus (leaf nodes in this tree) have a RintcToken.
      // Create a Cpu node.
      if (Generator->ProcNodeList[Index].RintcToken != CM_NULL_TOKEN) {
...
/** Create the processor hierarchy AML tree from CM_RISCV_PROC_HIERARCHY_INFO
    CM objects.
  ...
  )
{
  ...
  CM_RISCV_PROC_HIERARCHY_INFO   *ProcHierarchyNodeList;
  ...
  Status = GetERiscVObjProcHierarchyInfo (
             CfgMgrProtocol,
             CM_NULL_TOKEN,
  ...
  if (Status == EFI_NOT_FOUND) {
    ...
    Status = CreateTopologyFromRintc (
               Generator,
               CfgMgrProtocol,
               ScopeNode
               );
    ...
    // Generate the topology from CM_RISCV_PROC_HIERARCHY_INFO objects.
