/** @file
  RiscV PCI Configuration Space Parser.
...
#include "Pci/RiscVPciConfigSpaceParser.h"
  ...
  )
{
  ...
  CM_RISCV_PCI_ADDRESS_MAP_INFO  *PciAddressMapInfo;
  UINT32                       BufferSize;
  ...
  BufferSize        = Count * sizeof (CM_RISCV_PCI_ADDRESS_MAP_INFO);
  ...
  PciInfo->Mapping[PciMappingTableAddress].ObjectId =
    CREATE_CM_RISCV_OBJECT_ID (ERiscVObjPciAddressMapInfo);
  PciInfo->Mapping[PciMappingTableAddress].Size =
    sizeof (CM_RISCV_PCI_ADDRESS_MAP_INFO) * Count;
  ...
  )
{
  ...
  CM_RISCV_PCI_INTERRUPT_MAP_INFO  *PciInterruptMapInfo;
  UINT32                         BufferSize;
  ...
  IrqMapCount         = DataSize / IrqMapSize;
  BufferSize          = IrqMapCount * sizeof (CM_RISCV_PCI_ADDRESS_MAP_INFO);
  ...
  PciInfo->Mapping[PciMappingTableInterrupt].ObjectId =
    CREATE_CM_RISCV_OBJECT_ID (ERiscVObjPciInterruptMapInfo);
  PciInfo->Mapping[PciMappingTableInterrupt].Size =
    sizeof (CM_RISCV_PCI_INTERRUPT_MAP_INFO) * IrqMapCount;
...
/** Parse a Host-pci node.
  ...
  @param [in]       Fdt          Pointer to a Flattened Device Tree (Fdt).
  @param [in]       HostPciNode  Offset of a host-pci node.
  @param [in, out]  PciInfo      The CM_RISCV_PCI_CONFIG_SPACE_INFO to populate.
...
/** Add the parsed Pci information to the Configuration Manager.
  ...
  CmObj of the following types are concerned:
   - ERiscVObjPciConfigSpaceInfo
   - ERiscVObjPciAddressMapInfo
   - ERiscVObjPciInterruptMapInfo
...
{
  EFI_STATUS                    Status;
  CM_RISCV_PCI_CONFIG_SPACE_INFO  *PciConfigSpaceInfo;
  ...
  Status = AddSingleCmObj (
             FdtParserHandle,
             CREATE_CM_RISCV_OBJECT_ID (ERiscVObjPciConfigSpaceInfo),
             &PciTableInfo->PciConfigSpaceInfo,
             sizeof (CM_RISCV_PCI_CONFIG_SPACE_INFO),
             NULL
             );
...
/** CM_RISCV_PCI_CONFIG_SPACE_INFO parser function.
  ...
  typedef struct CmRiscVPciInterruptMapInfo {
    ...
    CM_RISCV_GENERIC_INTERRUPT    IntcInterrupt;    // {Populated}
