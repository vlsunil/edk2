  ...
  )
{
  ...
  Status = GetProcUid (ProcHierarchyNodeInfo->ApicToken, &Uid);
  if (EFI_ERROR (Status)) {
    ASSERT (0);
    return Status;
  }
  ...
  CpcToken = GetCpcToken (ProcHierarchyNodeInfo->ApicToken);
  ...
  EtToken = GetEtToken (ProcHierarchyNodeInfo->ApicToken);
...
ACPI_CPU_TOPOLOGY_GENERATOR  SsdtCpuTopologyGenerator = {
  // ACPI table generator header
  {
    ...
    0,
    // Creator ID
    TABLE_GENERATOR_CREATOR_ID_STD,
    // Creator Revision
