...
/** A structure used to handle the Lpi structures referencing.
  ...
  A CM_STD_PROC_HIERARCHY_INFO structure references a CM_STD_OBJ_REF.
  This CM_STD_OBJ_REF references CM_STD_LPI_INFO structures.

  Example:
  (Cpu0)                                   (Cpu1)
  CM_STD_PROC_HIERARCHY_INFO               CM_STD_PROC_HIERARCHY_INFO
              |                                       |
              +----------------------------------------
              |
              v
  ...
  CM_STD_LPI_INFO[0]                        CM_STD_LPI_INFO[1]

  Here, Cpu0 and Cpu1 have the same Lpi states. Both CM_STD_PROC_HIERARCHY_INFO
  structures reference the same CM_STD_OBJ_REF. An entry is created in the
  TokenTable such as:
  0 <-> CM_STD_OBJ_REF
...
typedef struct AcpiCpuTopologyGenerator {
  ...
  /// List of CM_STD_PROC_HIERARCHY_INFO CM objects.
  CM_STD_PROC_HIERARCHY_INFO    *ProcNodeList;
  /// Count of CM_STD_PROC_HIERARCHY_INFO CM objects.
