...
/** STD standard SSDT Pcie Table Generator.
...
GeneratePciDeviceInfo (
  IN      CONST CM_STD_PCI_CONFIG_SPACE_INFO  *PciInfo,
  IN            UINT32                        Uid,
  ...
  )
{
  ...
  CM_STD_OBJ_REF                 *RefInfo;
  UINT32                         RefCount;
  CM_STD_PCI_INTERRUPT_MAP_INFO  *IrqMapInfo;
  ...
  // Get the array of CM_STD_OBJ_REF referencing the
  // CM_STD_PCI_INTERRUPT_MAP_INFO objects.
  ...
  for (Index = 0; Index < RefCount; Index++) {
    // Get CM_STD_PCI_INTERRUPT_MAP_INFO structures one by one.
    Status = GetEStdObjPciInterruptMapInfo (
               CfgMgrProtocol,
  ...
  )
{
  ...
  CM_STD_OBJ_REF               *RefInfo;
  UINT32                       RefCount;
  CM_STD_PCI_ADDRESS_MAP_INFO  *AddrMapInfo;
  ...
  // Get the array of CM_STD_OBJ_REF referencing the
  // CM_STD_PCI_ADDRESS_MAP_INFO objects.
  Status = GetEStdObjCmRef (
             CfgMgrProtocol,
  ...
  for (Index = 0; Index < RefCount; Index++) {
    // Get CM_STD_PCI_ADDRESS_MAP_INFO structures one by one.
    Status = GetEStdObjPciAddressMapInfo (
               CfgMgrProtocol,
               RefInfo[Index].ReferenceToken,
               &AddrMapInfo,
               NULL
...
{
  EFI_STATUS                    Status;
  CM_STD_PCI_CONFIG_SPACE_INFO  *PciInfo;
  UINT32                        PciCount;
  UINTN                         Index;
  ...
  *TableCount = 0;
  ...
  Status = GetEStdObjPciConfigSpaceInfo (
             CfgMgrProtocol,
             CM_NULL_TOKEN,
             &PciInfo,
             &PciCount
