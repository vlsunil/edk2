...
/** Standard SSDT Cpu Topology Table Generator.

Requirements:
  The following Configuration Manager Object(s) are required by
  this Generator:
  - EArmObjGicCInfo
  - EStdObjProcHierarchyInfo (OPTIONAL) along with
  - EStdObjCmRef (OPTIONAL)
  - EStdObjLpiInfo (OPTIONAL)
...
/** Initialize the TokenTable.
  ...
  One entry should be allocated for each CM_STD_PROC_HIERARCHY_INFO
  structure of the platform. The TokenTable allows to have a mapping:
  Index <-> CM_OBJECT_TOKEN (to CM_STD_LPI_INFO structures).

  There will always be less sets of Lpi states (CM_STD_OBJ_REF)
  than the number of cpus/clusters (CM_STD_PROC_HIERARCHY_INFO).
...
{
  EFI_STATUS       Status;
  CM_STD_CPC_INFO  *CpcInfo;

  Status = GetEStdObjCpcInfo (
             CfgMgrProtocol,
             CpcToken,
             &CpcInfo,
             NULL
...
/** Create and add an _LPI method to Cpu/Cluster Node.
  ...
  @param [in]  Generator              The SSDT Cpu Topology generator.
  @param [in]  ProcHierarchyNodeInfo  CM_STD_PROC_HIERARCHY_INFO describing
                                      the Cpu.
...
CreateAmlLpiMethod (
  IN  ACPI_CPU_TOPOLOGY_GENERATOR  *Generator,
  IN  CM_STD_PROC_HIERARCHY_INFO   *ProcHierarchyNodeInfo,
  IN  AML_OBJECT_NODE_HANDLE       *Node
  )
{
  EFI_STATUS  Status;
  ...
  )
{
  ...
  CM_STD_OBJ_REF          *LpiRefInfo;
  UINT32                  LpiRefInfoCount;
  UINT32                  LpiRefIndex;
  CM_STD_LPI_INFO         *LpiInfo;
  ...
  for (Index = 0; Index < LastIndex; Index++) {
    ...
    for (LpiRefIndex = 0; LpiRefIndex < LpiRefInfoCount; LpiRefIndex++) {
      // For each CM_STD_LPI_INFO referenced by the token, add an Lpi state.
      Status = GetEStdObjLpiInfo (
                 CfgMgrProtocol,
...
/** Create a Cpu in the AML namespace from a CM_STD_PROC_HIERARCHY_INFO
    CM object.
  ...
  )
{
  ...
  ASSERT (ParentNode != NULL);
  ASSERT (ProcHierarchyNodeInfo != NULL);
  ASSERT (ProcHierarchyNodeInfo->ApicToken != CM_NULL_TOKEN);

  Status = GetProcUid (CfgMgrProtocol, ProcHierarchyNodeInfo->ApicToken, &Uid);
  if (EFI_ERROR (Status)) {
    ASSERT (0);
    return Status;
  }

  Status = CreateAmlCpu (Generator, ParentNode, Uid, CpuName, &CpuNode);
  if (EFI_ERROR (Status)) {
    ASSERT (0);
    return Status;
  }
  ...
  // CM_STD_PROC_HIERARCHY_INFO, create an _LPI method returning them.
  ...
  CpcToken = GetCpcToken (CfgMgrProtocol, ProcHierarchyNodeInfo->ApicToken);
  ...
  EtToken = GetEtToken (CfgMgrProtocol, ProcHierarchyNodeInfo->ApicToken);
...
/** Create a Processor Container in the AML namespace.
  ...
  Any CM_STD_PROC_HIERARCHY_INFO object with the following flags is
  ...
  @param [in]  ParentNode             Parent node to attach the processor
                                      container node to.
  @param [in]  ProcHierarchyNodeInfo  CM_STD_PROC_HIERARCHY_INFO object used
                                      to create the node.
...
/** Create an AML representation of the Cpu topology.
  ...
  @param [in] CfgMgrProtocol          Pointer to the Configuration Manager
                                      Protocol Interface.
  @param [in] NodeToken               Token of the CM_STD_PROC_HIERARCHY_INFO
                                      currently handled.
  ...
  )
{
  ...
  for (Index = 0; Index < Generator->ProcNodeCount; Index++) {
    // Find the children of the CM_STD_PROC_HIERARCHY_INFO
    ...
    if (Generator->ProcNodeList[Index].ParentToken == NodeToken) {
      ...
      // Create a Cpu node.
      if (Generator->ProcNodeList[Index].ApicToken != CM_NULL_TOKEN) {
...
/** Create the processor hierarchy AML tree from CM_STD_PROC_HIERARCHY_INFO
    CM objects.
  ...
  )
{
  ...
  CM_STD_PROC_HIERARCHY_INFO   *ProcHierarchyNodeList;
  ...
  Status = GetEStdObjProcHierarchyInfo (
             CfgMgrProtocol,
             CM_NULL_TOKEN,
  ...
  if (Status == EFI_NOT_FOUND) {
    ...
    // Generate the topology from CM_STD_PROC_HIERARCHY_INFO objects.
