  ...
  )
{
  ...
  // Check the CmObjDesc:
  //  - only RiscV objects are supported for now.
  //  - only ERiscVObjCmRef objects can be added as arrays.
  RiscVNamespaceObjId = GET_CM_OBJECT_ID (CmObjDesc->ObjectId);
  if ((CmObjDesc->Size == 0)              ||
      (CmObjDesc->Count == 0)             ||
      (RiscVNamespaceObjId >= ERiscVObjMax)   ||
      ((CmObjDesc->Count > 1)  && (RiscVNamespaceObjId != ERiscVObjCmRef))  ||
  ...
  // Add to link list.
  InsertTailList (&This->RiscVCmObjList[RiscVNamespaceObjId], &ObjNode->Link);
  This->ObjectCount += 1;
...
/** Group lists of CmObjNode from the RiscVNameSpace to one array.

  @param [in]  This         This dynamic platform repository.
  @param [in]  RiscVObjIndex  Index in ERISCV_OBJECT_ID
                            (must be < ERiscVObjMax).
  ...
  )
{
  ...
  if ((This == NULL)  ||
      (RiscVObjIndex >= ERiscVObjMax))
  {
DEBUG((DEBUG_INFO, "%a: RiscVObjIndex = %d\n", __func__, RiscVObjIndex));
    ASSERT (0);
    return EFI_INVALID_PARAMETER;
  }
  ...
  Count    = 0;
  Size     = 0;
  CmObjId  = CREATE_CM_RISCV_OBJECT_ID (RiscVObjIndex);
  ListHead = &This->RiscVCmObjList[RiscVObjIndex];
  ...
  while (Link != ListHead) {
    ...
    if ((CmObjDesc->Count != 1) && (RiscVObjIndex != ERiscVObjCmRef)) {
      // We expect each descriptor to contain an individual object.
      // ERiscVObjCmRef objects are counted as groups, so +1 as well.
      ASSERT (0);
      return EFI_INVALID_PARAMETER;
    }
  ...
  CmObjDesc           = &This->RiscVCmObjArray[RiscVObjIndex];
  ...
  )
{
  ...
  for (RiscVObjIndex = 0; RiscVObjIndex < ERiscVObjMax; RiscVObjIndex++) {
    Status = GroupCmObjNodes (This, RiscVObjIndex);
    if (EFI_ERROR (Status)) {
      ASSERT (0);
  ...
  )
{
  ...
  RiscVNamespaceObjId = GET_CM_OBJECT_ID (CmObjectId);
  ...
  if (RiscVNamespaceObjId == ERiscVObjCmRef) {
    // ERiscVObjCmRef object must be requested using a valid token.
    ASSERT (0);
    return EFI_INVALID_PARAMETER;
  }

  Desc = &This->RiscVCmObjArray[RiscVNamespaceObjId];
