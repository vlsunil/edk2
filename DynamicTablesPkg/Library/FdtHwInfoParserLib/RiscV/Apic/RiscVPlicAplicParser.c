...
/** Check if it is S-mode APLIC
  ...
  FDT will have entries for both M-mode and S-mode APLIC's. We
...
/** CM_ARCH_APLIC_INFO and CM_ARCH_PLIC_INFO parser function.
  ...
  for (Prev = 0; ; Prev = Node) {
    ...
    if (fdt_node_check_compatible (Fdt, AplicNode, "riscv,aplic") == 0) {
      if (IsSmodeAplic (Fdt, AplicNode)) {
        ZeroMem (&AplicInfo, sizeof (CM_ARCH_APLIC_INFO));
        ...
        Status = AddSingleCmObj (
                   FdtParserHandle,
                   CREATE_CM_ARCH_OBJECT_ID (EArchObjAplicInfo),
                   &AplicInfo,
                   sizeof (CM_ARCH_APLIC_INFO),
                   NULL
                   );
        if (EFI_ERROR (Status)) {
          ASSERT (0);
    ...
    if (fdt_node_check_compatible (Fdt, PlicNode, "riscv,plic0") == 0) {
      ...
      if (Prop && (Len >= 4) && (fdt32_to_cpu (Prop[1]) == IRQ_S_EXT)) {
        ZeroMem (&PlicInfo, sizeof (CM_ARCH_PLIC_INFO));
        ...
        Status = AddSingleCmObj (
                   FdtParserHandle,
                   CREATE_CM_ARCH_OBJECT_ID (EArchObjPlicInfo),
                   &PlicInfo,
                   sizeof (CM_ARCH_PLIC_INFO),
                   NULL
                   );
        if (EFI_ERROR (Status)) {
          ASSERT (0);
