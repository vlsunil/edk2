...
/** STD standard FADT Generator

Requirements:
  The following Configuration Manager Object(s) are required by
  this Generator:
  - EStdObjPowerManagementProfileInfo
  - EArmObjBootArchInfo
  - EStdObjHypervisorVendorIdentity (OPTIONAL)
*/

/** This macro defines the FADT flag options for STD Platforms.
*/
...
GET_OBJECT_LIST (
  EObjNameSpaceStd,
  EStdObjHypervisorVendorIdentity,
...
{
  EFI_STATUS                            Status;
  CM_STD_POWER_MANAGEMENT_PROFILE_INFO  *PmProfile;

  ASSERT (CfgMgrProtocol != NULL);

  // Get the Power Management Profile from the Platform Configuration Manager
  Status = GetEStdObjPowerManagementProfileInfo (
             CfgMgrProtocol,
             CM_NULL_TOKEN,
             &PmProfile,
             NULL
...
{
  EFI_STATUS                   Status;
  CM_STD_HYPERVISOR_VENDOR_ID  *HypervisorVendorInfo;

  ASSERT (CfgMgrProtocol != NULL);

  // Get the Hypervisor Vendor ID from the Platform Configuration Manager
  Status = GetEStdObjHypervisorVendorIdentity (
             CfgMgrProtocol,
             CM_NULL_TOKEN,
  ...
  DEBUG ((
    DEBUG_INFO,
    "FADT: EStdObjHypervisorVendorIdentity = 0x%lx\n",
...
{
  EFI_STATUS                  Status;
  CM_STD_FIXED_FEATURE_FLAGS  *FixedFeatureFlags;

  ASSERT (CfgMgrProtocol != NULL);

  // Get the Fixed feature flags from the Platform Configuration Manager
  Status = GetEStdObjFixedFeatureFlags (
             CfgMgrProtocol,
             CM_NULL_TOKEN,
             &FixedFeatureFlags,
             NULL
  ...
  DEBUG ((
    DEBUG_INFO,
    "FADT: EStdObjFixedFeatureFlags = 0x%x\n",
...
ACPI_TABLE_GENERATOR  FadtGenerator = {
  ...
  // Creator ID
  TABLE_GENERATOR_CREATOR_ID_EDK2,
  // Creator Revision
