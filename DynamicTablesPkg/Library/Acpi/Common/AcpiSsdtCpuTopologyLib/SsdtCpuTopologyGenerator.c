...
/** Create and add an Embedded trace device to the Cpu Node.
  ...
  @param [in]  CfgMgrProtocol         Pointer to the Configuration Manager
                                      Protocol Interface.
  @param [in]  AcpiProcessorUid       ACPI processor Uid of the local intc (gicc, rintc)
                                      describing the Cpu.
  ...
  )
{
  ...
  Status = GetIntCUidTokens(CfgMgrProtocol, ProcHierarchyNodeInfo->GicCToken,
                            &AcpiProcessorUid, &CpcToken, &EtToken);
  if (EFI_ERROR (Status)) {
    ASSERT (0);
    return Status;
  }
