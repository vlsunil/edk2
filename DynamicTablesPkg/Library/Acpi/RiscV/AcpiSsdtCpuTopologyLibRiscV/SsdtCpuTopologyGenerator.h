...
/** A structure used to handle the Lpi structures referencing.
  ...
  A CM_RISCV_PROC_HIERARCHY_INFO structure references a CM_ARM_OBJ_REF.
  This CM_RISCV_OBJ_REF references CM_ARM_LPI_INFO structures.

  Example:
  (Cpu0)                                   (Cpu1)
  CM_RISCV_PROC_HIERARCHY_INFO               CM_ARM_PROC_HIERARCHY_INFO
              |                                       |
              +----------------------------------------
              |
              v
  ...
  CM_RISCV_LPI_INFO[0]                        CM_ARM_LPI_INFO[1]

  Here, Cpu0 and Cpu1 have the same Lpi states. Both CM_RISCV_PROC_HIERARCHY_INFO
  structures reference the same CM_RISCV_OBJ_REF. An entry is created in the
  TokenTable such as:
  0 <-> CM_RISCV_OBJ_REF
...
typedef struct AcpiCpuTopologyGenerator {
  ...
  /// List of CM_RISCV_PROC_HIERARCHY_INFO CM objects.
  CM_RISCV_PROC_HIERARCHY_INFO    *ProcNodeList;
  /// Count of CM_RISCV_PROC_HIERARCHY_INFO CM objects.
