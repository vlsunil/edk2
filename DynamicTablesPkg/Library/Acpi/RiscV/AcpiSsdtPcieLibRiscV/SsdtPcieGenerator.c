...
/** RISCV standard SSDT Pcie Table Generator.
...
GeneratePciDeviceInfo (
  IN      CONST CM_RISCV_PCI_CONFIG_SPACE_INFO  *PciInfo,
  IN            UINT32                        Uid,
  ...
  )
{
  ...
  CM_RISCV_OBJ_REF                 *RefInfo;
  UINT32                         RefCount;
  CM_RISCV_PCI_INTERRUPT_MAP_INFO  *IrqMapInfo;
  ...
  // Get the array of CM_RISCV_OBJ_REF referencing the
  // CM_RISCV_PCI_INTERRUPT_MAP_INFO objects.
  ...
  for (Index = 0; Index < RefCount; Index++) {
    // Get CM_RISCV_PCI_INTERRUPT_MAP_INFO structures one by one.
    Status = GetERiscVObjPciInterruptMapInfo (
               CfgMgrProtocol,
    ...
    // Cf. RiscV BSA v1.0, sE.6 "Legacy interrupts"
    if ((Index > 0)   &&
  ...
  Status = GeneratePciSlots (&Generator->DeviceTable, Uid, PciNode);
  if (EFI_ERROR (Status)) {
    ASSERT (0);
    goto exit_handler;
  }
  ...
  )
{
  ...
  CM_RISCV_OBJ_REF               *RefInfo;
  UINT32                       RefCount;
  CM_RISCV_PCI_ADDRESS_MAP_INFO  *AddrMapInfo;
  ...
  // Get the array of CM_RISCV_OBJ_REF referencing the
  // CM_RISCV_PCI_ADDRESS_MAP_INFO objects.
  Status = GetERiscVObjCmRef (
             CfgMgrProtocol,
  ...
  for (Index = 0; Index < RefCount; Index++) {
    // Get CM_RISCV_PCI_ADDRESS_MAP_INFO structures one by one.
    Status = GetERiscVObjPciAddressMapInfo (
               CfgMgrProtocol,
               RefInfo[Index].ReferenceToken,
               &AddrMapInfo,
               NULL
...
{
  EFI_STATUS                    Status;
  CM_RISCV_PCI_CONFIG_SPACE_INFO  *PciInfo;
  UINT32                        PciCount;
  UINTN                         Index;
  ...
  *TableCount = 0;
  ...
  Status = GetERiscVObjPciConfigSpaceInfo (
             CfgMgrProtocol,
             CM_NULL_TOKEN,
             &PciInfo,
             &PciCount
