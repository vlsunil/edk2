...
UINT32
GetCmoNodeSize (
  IN  CONST CM_ARCH_CMO_NODE  *Node
  )
{
  ASSERT (Node != NULL);
...
GetSizeofCmoNodes (
  ...
  IN      CONST CM_ARCH_CMO_NODE          *NodeList,
  IN            UINT32                         NodeCount,
...
GetSizeofHartInfoNodes (
  IN      CONST UINT32                              NodeStartOffset,
  IN      CONST CM_ARCH_RINTC_INFO            *NodeList,
  IN            UINT32                              NodeCount,
  IN            UINT32                              NumOffsets,
  IN OUT        RHCT_NODE_INDEXER          **CONST  NodeIndexer
  )
...
DEBUG((DEBUG_INFO, "%a: HartInfoNode = 0x%llx, NodeList->AcpiProcessorUid=0x%x, NodeLength=%d\n", __func__, HartInfoNode, NodeList->AcpiProcessorUid, NodeLength));
    ...
    for (i = 0; i < NumOffsets; i++) {
      HartInfoNode->Offsets[i] = Offsets[i];
    }
  ...
  )
{
  ...
  UINT32  IsaStringNodeCount, IsaStringOffset;
  UINT32  CmoNodeCount, CmoOffset;
  UINT32  HartInfoNodeCount, HartInfoOffset;
  UINT32  TimerInfoCount;

  CM_ARCH_RINTC_INFO        *RintcInfoNodeList;
  CM_ARCH_CMO_NODE  *CmoNodeList;
  //CHAR8  *IsaString;
  CM_ARCH_TIMER_INFO        *TimerInfo;
  CM_ARCH_ISA_STRING_NODE        *IsaStringNode;
  UINT32 *Offsets;
  UINTN i;
  ...
  *Table    = NULL;
  ...
  Status = GetEArchObjIsaStringInfo (
             CfgMgrProtocol,
             CM_NULL_TOKEN,
             &IsaStringNode,
             &IsaStringNodeCount
  ...
  *Table = (EFI_ACPI_DESCRIPTION_HEADER *)AllocateZeroPool (TableSize);
  ...
  i = 0;
  if (IsaStringNodeCount) {
    Offsets[i++] = IsaStringOffset;
  }
  if (CmoNodeCount) {
    Offsets[i++] = CmoOffset;
  }
