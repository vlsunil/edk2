...
#include "Serial/StdSerialPortParser.h"
...
/** Parse a serial port node.
  ...
  @param [in]  SerialPortInfo    The CM_STD_SERIAL_PORT_INFO to populate.
...
SerialPortNodeParser (
  ...
  IN  CM_STD_SERIAL_PORT_INFO  *SerialPortInfo
  )
{
  EFI_STATUS   Status;
  INT32        IntcNode;
...
/** CM_STD_SERIAL_PORT_INFO dispatcher function (for a generic serial-port).
...
EFI_STATUS
EFIAPI
StdSerialPortInfoDispatch (
  IN  CONST FDT_HW_INFO_PARSER_HANDLE  FdtParserHandle,
  IN  CM_STD_SERIAL_PORT_INFO          *GenericSerialInfo,
  IN  INT32                            NodeCount,
  IN  ESTD_OBJECT_ID                   SerialObjectId
  )
{
  EFI_STATUS         Status;
  ...
  if ((SerialObjectId != EStdObjSerialPortInfo) &&
      (SerialObjectId != EStdObjSerialDebugPortInfo) &&
      (SerialObjectId != EStdObjSerialConsolePortInfo))
  {
    ASSERT (0);
    return EFI_INVALID_PARAMETER;
  }
  ...
  Status = CreateCmObjDesc (
             CREATE_CM_STD_OBJECT_ID (SerialObjectId),
             NodeCount,
             GenericSerialInfo,
             sizeof (CM_STD_SERIAL_PORT_INFO) * NodeCount,
             &NewCmObjDesc
             );
  if (EFI_ERROR (Status)) {
    ASSERT (0);
...
/** CM_STD_SERIAL_PORT_INFO parser function (for debug/console serial-port).
  ...
  @param [in]  SerialObjectId  StdNamespace Object ID for the serial port.
...
{
  EFI_STATUS               Status;
  CM_STD_SERIAL_PORT_INFO  SerialInfo;

  if ((SerialObjectId != EStdObjSerialDebugPortInfo) &&
      (SerialObjectId != EStdObjSerialConsolePortInfo))
  {
    ASSERT (0);
    return EFI_INVALID_PARAMETER;
  ...
  Status = StdSerialPortInfoDispatch (
             FdtParserHandle,
             &SerialInfo,
             1,
             SerialObjectId
...
/** SerialPort dispatcher.
  ...
  This disptacher populates the CM_STD_SERIAL_PORT_INFO structure for
  the following CM_OBJ_ID:
   - EStdObjSerialConsolePortInfo
   - EStdObjSerialDebugPortInfo
  ...
  )
{
  ...
  CM_STD_SERIAL_PORT_INFO  *GenericSerialInfo;
  ...
  if (Status == EFI_NOT_FOUND) {
    ...
    Status = StdSerialPortInfoParser (
               FdtParserHandle,
               SerialConsoleNode,
               EStdObjSerialConsolePortInfo
               );
  ...
  if (SerialNodesRemaining > 1) {
    ...
    GenericSerialInfo = AllocateZeroPool (
                          SerialNodesRemaining *
                          sizeof (CM_STD_SERIAL_PORT_INFO)
                          );
  ...
  for (Index = 0; Index < SerialNodeCount; Index++) {
    ...
    if (SerialNode == SerialConsoleNode) {
      ...
      Status = StdSerialPortNodeParser (
                 Fdt,
                 SerialNode,
