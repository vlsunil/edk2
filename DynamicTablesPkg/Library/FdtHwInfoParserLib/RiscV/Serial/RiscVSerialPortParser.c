...
#include "Serial/RiscVSerialPortParser.h"
...
/** Parse a serial port node.
  ...
  @param [in]  SerialPortInfo    The CM_RISCV_SERIAL_PORT_INFO to populate.
...
SerialPortNodeParser (
  ...
  IN  CM_RISCV_SERIAL_PORT_INFO  *SerialPortInfo
  )
{
  EFI_STATUS   Status;
  INT32        IntcNode;
...
/** CM_RISCV_SERIAL_PORT_INFO dispatcher function (for a generic serial-port).
...
EFI_STATUS
EFIAPI
RiscVSerialPortInfoDispatch (
  IN  CONST FDT_HW_INFO_PARSER_HANDLE  FdtParserHandle,
  IN  CM_RISCV_SERIAL_PORT_INFO          *GenericSerialInfo,
  IN  INT32                            NodeCount,
  IN  ERISCV_OBJECT_ID                   SerialObjectId
  )
{
  EFI_STATUS         Status;
  ...
  if ((SerialObjectId != ERiscVObjSerialPortInfo) &&
      (SerialObjectId != ERiscVObjSerialDebugPortInfo) &&
      (SerialObjectId != ERiscVObjSerialConsolePortInfo))
  {
    ASSERT (0);
    return EFI_INVALID_PARAMETER;
  }
  ...
  Status = CreateCmObjDesc (
             CREATE_CM_RISCV_OBJECT_ID (SerialObjectId),
             NodeCount,
             GenericSerialInfo,
             sizeof (CM_RISCV_SERIAL_PORT_INFO) * NodeCount,
             &NewCmObjDesc
             );
  if (EFI_ERROR (Status)) {
    ASSERT (0);
...
/** CM_RISCV_SERIAL_PORT_INFO parser function (for debug/console serial-port).
  ...
  @param [in]  SerialObjectId  RiscVNamespace Object ID for the serial port.
...
{
  EFI_STATUS               Status;
  CM_RISCV_SERIAL_PORT_INFO  SerialInfo;

  if ((SerialObjectId != ERiscVObjSerialDebugPortInfo) &&
      (SerialObjectId != ERiscVObjSerialConsolePortInfo))
  {
    ASSERT (0);
    return EFI_INVALID_PARAMETER;
  ...
  Status = RiscVSerialPortInfoDispatch (
             FdtParserHandle,
             &SerialInfo,
             1,
             SerialObjectId
...
/** SerialPort dispatcher.
  ...
  This disptacher populates the CM_RISCV_SERIAL_PORT_INFO structure for
  the following CM_OBJ_ID:
   - ERiscVObjSerialConsolePortInfo
   - ERiscVObjSerialDebugPortInfo
  ...
  )
{
  ...
  CM_RISCV_SERIAL_PORT_INFO  *GenericSerialInfo;
  ...
  if (Status == EFI_NOT_FOUND) {
    ...
    Status = RiscVSerialPortInfoParser (
               FdtParserHandle,
               SerialConsoleNode,
               ERiscVObjSerialConsolePortInfo
               );
  ...
  if (SerialNodesRemaining > 1) {
    ...
    GenericSerialInfo = AllocateZeroPool (
                          SerialNodesRemaining *
                          sizeof (CM_RISCV_SERIAL_PORT_INFO)
                          );
