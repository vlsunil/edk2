/** @file
  Std PCI Configuration Space Parser.
  ...
  )
{
  ...
  CM_STD_PCI_ADDRESS_MAP_INFO  *PciAddressMapInfo;
  UINT32                       BufferSize;
  ...
  BufferSize        = Count * sizeof (CM_STD_PCI_ADDRESS_MAP_INFO);
  ...
  PciInfo->Mapping[PciMappingTableAddress].ObjectId =
    CREATE_CM_STD_OBJECT_ID (EStdObjPciAddressMapInfo);
  PciInfo->Mapping[PciMappingTableAddress].Size =
    sizeof (CM_STD_PCI_ADDRESS_MAP_INFO) * Count;
  ...
  )
{
  ...
  CM_STD_PCI_INTERRUPT_MAP_INFO  *PciInterruptMapInfo;
  UINT32                         BufferSize;
...
DEBUG((DEBUG_INFO, "%a: DataSize=%d, PciIntCells=%d, IntcAddressCells=%d, IntcCells=%d\n", __func__, DataSize, PciIntCells, IntcAddressCells, IntcCells));
  ...
  IrqMapCount         = DataSize / IrqMapSize;
  BufferSize          = IrqMapCount * sizeof (CM_STD_PCI_ADDRESS_MAP_INFO);
...
DEBUG((DEBUG_INFO, "%a: PciInterruptMapInfo[Index].IntcInterrupt.Interrupt = %d\n", __func__, PciInterruptMapInfo[Index].IntcInterrupt.Interrupt));
DEBUG((DEBUG_INFO, "%a: PciInterruptMapInfo[Index].IntcInterrupt.Flags = %d\n", __func__, PciInterruptMapInfo[Index].IntcInterrupt.Flags));
  } // for

  PciInfo->Mapping[PciMappingTableInterrupt].ObjectId =
    CREATE_CM_STD_OBJECT_ID (EStdObjPciInterruptMapInfo);
  PciInfo->Mapping[PciMappingTableInterrupt].Size =
    sizeof (CM_STD_PCI_INTERRUPT_MAP_INFO) * IrqMapCount;
...
/** Parse a Host-pci node.
  ...
  @param [in]       Fdt          Pointer to a Flattened Device Tree (Fdt).
  @param [in]       HostPciNode  Offset of a host-pci node.
  @param [in, out]  PciInfo      The CM_STD_PCI_CONFIG_SPACE_INFO to populate.
...
/** Add the parsed Pci information to the Configuration Manager.
  ...
  CmObj of the following types are concerned:
   - EStdObjPciConfigSpaceInfo
   - EStdObjPciAddressMapInfo
   - EStdObjPciInterruptMapInfo
...
{
  EFI_STATUS                    Status;
  CM_STD_PCI_CONFIG_SPACE_INFO  *PciConfigSpaceInfo;
  ...
  Status = AddSingleCmObj (
             FdtParserHandle,
             CREATE_CM_STD_OBJECT_ID (EStdObjPciConfigSpaceInfo),
             &PciTableInfo->PciConfigSpaceInfo,
             sizeof (CM_STD_PCI_CONFIG_SPACE_INFO),
             NULL
             );
...
/** CM_STD_PCI_CONFIG_SPACE_INFO parser function.
  ...
  typedef struct CmStdPciInterruptMapInfo {
    ...
    CM_STD_GENERIC_INTERRUPT    IntcInterrupt;    // {Populated}
